...
class ModelArguments:
    ...
    out_domain: bool = field(
        default=False,
        metadata={"help": "whether to use out domain attributes for eval."},
    )
...
def main():
    ...
    def compute_metrics_multi_attr(eval_preds):
        ...
        if not model_args.out_domain:
            with open('attributes_merged.txt') as infile:
                attributes = [line.strip().split(',')[0] for line in infile.readlines()]
        else:
            with open('attributes_out_domain.txt') as infile:
                attributes = [line.strip().split(',')[0] for line in infile.readlines()]
